...
class Server {
  ...
  getMatchesByQueryString(req, bundle) {
    ...
    bundle.forEach(record => {
      ...
      let result = this.getMatchingResultByQueryStrings(
        req.query,
  ...
  cb(req, res) {
    ...
    if (bundle.length === 1) {
      ...
      if (response === null) {
        ...
        console.log(
          colors.bgBlue.white('I know this URL but no match for parameters.')
        );
    ...
    Object.keys(map).forEach(path => {
      ...
      if (patches) {
        patches.forEach(record => {
          this.app.patch(path, this.cb.bind(this));
          this.logRegistredPath(isLogEnabled, methods.PATCH, path);
        });
      }
    });
  }
